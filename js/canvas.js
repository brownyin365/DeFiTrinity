"use strict";
{
	...
	class Particle {
		...
		init() {
			...
			this.y0 = canvas.height * 0.5 + this.j * canvas.height / 60;
		}
		move() {
			...
			const s = 1500 / d;
			this.x += -s * (dx / d) + (this.x0 - this.x) * 0.01;
			this.y += -s * (dy / d) + (this.y0 - this.y) * 0.01;
		...
		init(options) {
			...
			gl.shaderSource(
				...
				`
					...
					void main() {
						gl_PointSize = max(0.5, min(30.0, aPosition.z));
						gl_Position = vec4(
			...
			gl.shaderSource(
				...
				`
					...
					void main() {
						...
						gl_FragColor = vec4(1.0, 0.86, 1.0, 1.0 - dot(pc, pc));
					}
				`
			);
